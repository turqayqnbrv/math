#!/usr/bin/env python3
'''All previous plots in the same task'''
import numpy as np
import matplotlib.pyplot as plt


def all_in_one():
    '''Plotting all previous tasks' results'''
    y0 = np.arange(0, 11) ** 3
    ...
    ax0.plot(y0, 'r-')
    ax0.set_xlim([0, 10])
    ax0.set_title('Exponential Decay', fontsize='x-small')
    ax0.set_xlabel('X', fontsize='x-small')
    ax0.set_ylabel('Y', fontsize='x-small')

    ax1 = plt.subplot2grid(grid_shape, (0, 1))
    ax1.scatter(x1, y1, c='magenta', s=5)
    ax1.set_xlabel('Height (in)', fontsize='x-small')
    ax1.set_ylabel('Weight (lbs)', fontsize='x-small')
    ax1.set_title('Height vs Weight', fontsize='x-small')

    ax2 = plt.subplot2grid(grid_shape, (1, 0))
    ax2.plot(x2, y2)
    ax2.set_yscale('log')
    ...
    ax4.hist(student_grades, bins=list(range(0, 101, 10)), edgecolor='black')
    ax4.set_xlim([0, 100])
    ax4.set_ylim([0, 30])
